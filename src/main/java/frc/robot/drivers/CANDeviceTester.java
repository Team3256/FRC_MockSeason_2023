...
public class CANDeviceTester {
  ...
  public static boolean testTalonFX(TalonFX device) {
    StatusSignal temp = device.getDeviceTemp();
    double tempDub = (double) temp.getValue();
    if (tempDub == 0) System.out.println("TalonFX " + device.getDeviceID() + " offline");
    return tempDub != 0;
  }
  ...
  public static boolean testPigeon(Pigeon2 device) {
    StatusSignal temp = device.getTemperature();
    double tempDub = (double) temp.getValue();
    if (tempDub == 0) System.out.println("Pigeon " + device.getDeviceID() + " offline");
    return tempDub != 0;
  }
  ...
  public static boolean testCANCoder(CANcoder device) {
    StatusSignal voltage = device.getSupplyVoltage();
    double voltDub = (double) voltage.getValue();
    if (voltDub == 0) System.out.println("CANCoder " + device.getDeviceID() + " offline");
    return voltDub != 0;
  }
}